            ...
            <div class="col-md-12">
                <div class="card">
                    <div class="card-header bg-success">{{ $title }}
                        ...
                        <a target="_blank" href="{{ route('Tag.create') }}" class="pull-left btn btn-primary margin-top-n-4 text-white ml-4 px-3 mt-0 pt-1 pb-0">
                                ...
                                @foreach($query as $item)
                                    <tr class="gradeX data{{ $item->id }}">
                                        ...
                                        <td class="align-middle">
                                            ...
                                            <button title="حذف" href="#" class="btn btn-danger btn-sm" id="del{{ $item->id }}" data-url="{{ url('Admin/Tag/'.$item->id) }}" onclick="del({{ $item->id }});">
                                                <i class="fa fa-trash"></i>
                                            </button>
                                            @endcan
                                            @can(\App\Helper\Helper::getTypePermission('edit'))
                                            <a title="ویرایش" href="{{ url('Admin/Tag/'.$item->id.'/edit') }}" class="btn btn-outline-success btn-sm">
