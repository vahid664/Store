            ...
            <div class="col-md-12">
                <div class="card">
                    <div class="card-header bg-success">{{ $title }}
                        ...
                        <a target="_blank" href="{{ url('Admin/Gift/create') }}" class="pull-left btn btn-primary margin-top-n-4 text-white ml-4 px-3 mt-0 pt-1 pb-0">
                            ...
                            <table class="table table-striped table-bordered table-hover dataTables-example" >
                                ...
                                @foreach($query as $item)
                                    <tr class="gradeX data{{ $item->id }}">
                                        ...
                                        <td class="align-middle">{{ $item->product->title }} - {{ $item->product->title_en }}</td>
                                        <td class="align-middle">{{ $item->title }}</td>
                                        <td class="align-middle">{{ $item->count }}</td>
                                        <td class="align-middle">{{ $item->count_use }}</td>
                                        <td class="align-middle">{{ $item->floor_price_basket }}</td>
                                        ...
                                        <td class="align-middle">
                                            ...
                                            <button title="حذف" href="#" class="btn btn-danger btn-sm" id="del{{ $item->id }}" data-url="{{ url('Admin/Gift/'.$item->id) }}" onclick="del({{ $item->id }});">
                                                <i class="fa fa-trash"></i>
                                            </button>
                                            @endcan
                                            @can(\App\Helper\Helper::getTypePermission('edit'))
                                            <a title="ویرایش" href="{{ url('Admin/Gift/'.$item->id.'/edit') }}" class="btn btn-outline-success btn-sm">
